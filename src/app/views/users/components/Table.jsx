...
const Table = () => {
    ...
    return (
        <Card elevation={3} sx={{pt: '20px', mb: 3}}>
                    ...
                    <TableBody>
                        {userList.map((user, index) => (
                            <TableRow key={index} hover>
                                ...
                                <TableCell sx={{px: 0}} colSpan={1}>
                                    <Tooltip title="Edit">
                                        <IconButton onClick={() => handleForm(user.id)}>
                                            <Icon color="primary">edit</Icon>
                                        </IconButton>
                                    </Tooltip>
                                    <Tooltip title="Delete">
                                        <IconButton onClick={() => handleForm(user.id, 'delete')}>
            ...
            <Delete domain={'users'} formData={formData} handleCloseForm={(refresh) => handleRefresh(refresh)}/>
        </Card>
    )
}
